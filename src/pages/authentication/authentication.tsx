...
const Authentication: React.FC = () => {
  const { handleLogin, userData, loading, setLoading, setAlertData } =
    useAppContext();
  ...
  const verifyInputs = () => {
    ...
    if (
      !("password" in userAuthentication) ||
    ...
    if (verifyInputs()) {
      ...
      try {
        const result = await handleLogin(
          userAuthentication as UserAuthenticationObject
        );
  ...
  return (
    <div className="min-h-screen flex items-center justify-center py-8 pb-20">
      <div className="z-[-1] fixed top-0 left-0 w-full h-full bg-cover bg-no-repeat bg-[url('/background/influencer-background.jpeg')] blur-md" />
      <div className="flex">
        <Card gap={2}>
          ...
          <div>
            <form className="mb-6">
              ...
              <span className="text-gray-700 text-light">
                Não possuí uma conta?{" "}
                <a href="/register" className="text-primary">
                  Cadastre-se agora
                </a>
              </span>
              <span className="flex text-primary justify-center w-full mt-3">
                Esqueci minha senha
              </span>
            </form>
          </div>
