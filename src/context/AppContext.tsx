        ...
        if (token != "null") {
          ...
          const timeout = new Promise((_, reject) =>
            setTimeout(
              () =>
                reject(
                  new Error("Timeout: A API demorou muito para responder")
                ),
              TIMEOUT_MS
            )
          );
          ...
          if (success) {
            ...
            const isPermissionPaying = user.permissions.includes("admin")
              ? true
              : user.paying
              ? true
              : false;
  ...
  const handleLogin = async (userAuthentication: UserAuthentication) => {
    try {
      ...
      if (data.success) {
        ...
        const isPermissionPaying = data.user.permissions.includes("admin")
          ? true
          : data.user.paying
          ? true
          : false;
    ...
    >
      ...
      <div className="flex">
        <CryptoModal
          ...
          type={alertData?.type}
          isOpen={alertData?.active}
          closeModal={() =>
            setAlertData({ active: false, title: "", message: "", type: "" })
          }
        >
